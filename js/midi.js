...
jvtool.midi = (function () {
  ...
  my.useControllerIn = function (portName) {
    ...
    if (port) {
      controllerIn = port;
      controllerIn.onmidimessage = onMessage;
  ...
  my.getInNames = () => midiInPorts.map((port) => port.name);

  my.getOutNames = () => midiOutPorts.map((port) => port.name);
  ...
  my.sendMessage = function (bytes) {
    let hs = jvtool.util.toHexStrings(bytes);
    console.log(bytes, "->", hs);
    console.log("Sent: [" + hs + "]");
    if (!midiOut) return;
    midiOut.send(bytes);
  };
